...
$$\text{Momentum}_{20d}(t, i) = \frac{P_{\text{close}}(t, i) - P_{\text{close}}(t-20, i)}{P_{\text{close}}(t-20, i)}$$
...
$$\text{VolAdjMom}_{20d}(t, i) = \frac{\text{Momentum}_{20d}(t, i)}{\sigma_{20d}(t, i)}$$
...
$$\text{Reversal}_{5d}(t, i) = -\text{Momentum}_{5d}(t, i) = -\frac{P_{\text{close}}(t, i) - P_{\text{close}}(t-5, i)}{P_{\text{close}}(t-5, i)}$$
...
$$\sigma_{20d}(t, i) = \sqrt{\frac{1}{19}\sum_{k=t-19}^{t} \left(r_k(i) - \bar{r}(i)\right)^2}$$

$$r_k(i) = \frac{P_k(i) - P_{k-1}(i)}{P_{k-1}(i)}$$
...
$$\text{Turnover}(t, i) = \frac{1}{20}\sum_{k=t-19}^{t} \frac{\text{Volume}_k(i)}{\text{Shares Outstanding}_k(i)}$$

若tushare积分不够导致`daily_basic` 接口用不了，可考虑估计

$$\text{ApproxTurnover}_{20d}(t, i) = \frac{1}{20}\sum_{k=t-19}^{t} \frac{\text{Volume}_k(i) - \text{Volume}_{k-1}(i)}{\text{Volume}_{k-1}(i)}$$
...
$$\text{Amount}_{20d}(t, i) = \frac{1}{20}\sum_{k=t-19}^{t} \text{Amount}_k(i)$$
...
$$\text{RS}(t, i) = \frac{\text{AvgGain}_{14d}(t, i)}{\text{AvgLoss}_{14d}(t, i)}$$

其中
$$\text{AvgGain}_{14d} = \frac{1}{14}\sum_{k=t-13}^{t} \max(\Delta P_k, 0)$$
$$\text{AvgLoss}_{14d} = \frac{1}{14}\sum_{k=t-13}^{t} \max(-\Delta P_k, 0)$$

$$\text{RSI}_{14d}(t, i) = 100 - \frac{100}{1 + \text{RS}(t, i)}$$

因子值取

$$\text{RSIFactor}(t, i) = \frac{\text{RSI}_{14d}(t, i) - 50}{50}$$
...
$$\text{MA}_{20d}(t, i) = \frac{1}{20}\sum_{k=t-19}^{t} P_k(i)$$

$$\text{BIAS}_{20d}(t, i) = \frac{P_t(i) - \text{MA}_{20d}(t, i)}{\text{MA}_{20d}(t, i)}$$
